...
def generate_json():
    ...
    try:
        ...
        c.execute('''
            SELECT a.*, l.lead_time_days, l.ob1_detection_date, l.mainstream_source
            FROM anomalies a
        ...
        for row in rows:
            ...
            if url and url != 'N/A':
                ...
                item['sources'] = []
            item.setdefault('detection_count', 1)
            item.setdefault('first_detected', item.get('detection_date'))
            data.append(item)
